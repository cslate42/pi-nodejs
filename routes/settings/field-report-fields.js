...
router.all('/add', function (req, res, next) {
    global.preparePageData(req, res, next);
    var queryResults = global.dbGetTableColumns(global.DB_TABLE_DAMAGE_REPORT_FIELDS);
    ...
    if ( req.body.submitted && msgs.length === 0 ) {
        ...
        var positions = global.dbSelect(global.DB_TABLE_DAMAGE_REPORT_FIELDS, 'MAX(`position`) AS `max_pos`', [{col:'phase_id', val:fieldObj.phase_id}]);
        ...
        if (errors.length === 0) {
            global.dbInsert(global.DB_TABLE_DAMAGE_REPORT_FIELDS, fieldObj); 
...
//    console.log(data);
    res.render('settings/damage-report-fields/add', data);
});

router.all('/delete/:id', function (req, res, next) {
    var id = parseInt(req.params.id);
    global.dbDelete(global.DB_TABLE_DAMAGE_REPORT_FIELDS, [{col:'id', val:id}]);
    global.dbDelete(global.DB_TABLE_DAMAGE_REPORT_DATA, [{col:'damage_report_field_id', val:id}]);
    res.redirect('/settings/damage-report-fields/');
    res.end();
});
...
router.all('/edit/:id', function (req, res, next) {
    ...
    if ( req.body.submitted && msgs.length === 0 ) {
        ...
        global.dbUpdate(global.DB_TABLE_DAMAGE_REPORT_FIELDS, values, [{col: 'id', val: id}]);
        msgs.push( sprintf("Successfully edited %s", values.name) );
    }
    
    var queryResults = global.dbSelect(global.DB_TABLE_DAMAGE_REPORT_FIELDS, '*', [{col: 'id', val: id}]);
    ...
    if (queryResults.rowCount === 1) {
        ...
        var data = {
            ...
            deleteUrl: sprintf("/settings/damage-report-fields/delete/%d/", id),
        };
        //console.log(data);
        res.render('settings/damage-report-fields/edit', data);
    } else {
        res.redirect('/settings/damage-report-fields/');
        res.end();
    }
...
router.all('/get_list', function (req, res, next) {
    ...
    var queryResults = global.dbSelect(global.DB_TABLE_DAMAGE_REPORT_FIELDS, '*', null, start, global.ROWS_PER_PAGE);
...
//    console.log(data);
    res.render('settings/damage-report-fields/ajax/list', data);
});
...
router.all('/', function (req, res, next) {
    ...
    if (global.isRedirecting !== false) {
        ...
        res.render('settings/damage-report-fields/table', data);
    }
});
