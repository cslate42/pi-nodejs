...
router.all('/edit/:id', function (req, res, next) {
    ...
    if (queryResults.rowCount === 1) {
        var currentFieldsPositionHTML = "<p>Select position of field that you would like to swap with</p>";
        var fieldDataRaw = {};
        if (queryResults.rowCount === 1) {
        ...
        Object.keys(fieldDataRaw).forEach(function (key) {
            switch (key) {
                ...
                case "position":
                    var options = [];
                    var positions = global.dbSelect(global.DB_TABLE_PHASE_FIELDS, '`id`, `name`, `position`', [{col:'phase_id', val:fieldDataRaw.phase_id}], null, null, 'position').rows;
                    for(var i = 0; i < positions.length; i++ ){
                        options.push({
                            value: positions[i].position,
                            text: positions[i].position,
                        });
                        var isCurrentPosition = parseInt(positions[i].id) === id;
                        if( isCurrentPosition ) {
                            currentFieldsPositionHTML += sprintf("<p>%d - <i>CURRENT</i> - <b>%s</b></p>", positions[i].position, positions[i].name);
                        } else {
                            currentFieldsPositionHTML += sprintf("<p>%d - %s</p>", positions[i].position, positions[i].name);
                        }
                    }
                    fieldData[key] = {
                        selected: fieldDataRaw[key],
        ...
        var data = {
            ...
            submitValue: 'Add',
            currentFieldsPositionHTML: currentFieldsPositionHTML,
        };
        //console.log(data);
...
router.all('/get_list', function (req, res, next) {
    ...
    var queryResults = global.dbSelect(
            ...
            null,
            null,
            " GROUP BY `phase_id` ORDER BY `position` ASC "
    );
