...
router.all('/add', function (req, res, next) {
    ...
    if (req.body.submitted && msgs.length === 0) {
        ...
        //save uploaded field media
        var mediaData = {
            resource_id: projectId,
        };
        var mediaWhere = [
            {col:'tmp_identifiers', val: file.getTmpFieldMediaIdentifer(req.session.uid) },
            {col:'resource', val: 'project'},
        ];
        global.dbUpdate(global.DB_TABLE_MEDIA, mediaData, mediaWhere);
...
router.all('/get_list', function (req, res, next) {
    ...
    var whereArgs = [];
    var filterOptions = {};
    for( var index in req.body.filterOptions ) {
        var key = req.body.filterOptions[index].name.replace("[]", '');
        var value = req.body.filterOptions[index].value;
        if ( typeof filterOptions[key] === 'undefined' ) {
            filterOptions[key] = value;
        } else if ( typeof filterOptions[key] === 'object' ) {
            filterOptions[key].push(value);
        } else {
            filterOptions[key] = [ filterOptions[key], value ];
        }
    }
    ...
    for( var key in filterOptions ) {
        whereArgs.push({
            col: key,
            val: filterOptions[key],
        });
    }
    
    var queryResults = global.dbSelect(global.DB_TABLE_PROJECTS, '*', whereArgs, start, global.ROWS_PER_PAGE);
...
router.all('/', function (req, res, next) {
    ...
    var states = global.dbSelect(global.DB_TABLE_PROJECTS, ' DISTINCT `state` as `name`').rows;
    var cities = global.dbSelect(global.DB_TABLE_PROJECTS, ' DISTINCT `city` as `name`').rows;
    var phases = global.dbSelect(global.DB_TABLE_PHASES, '`id`, `name`').rows;
    var managerRows = global.dbSelect(global.DB_TABLE_PROJECTS, ' DISTINCT `manager_id` as `id`').rows;
    
    var managerArgs = [];
    for( var i = 0; i < managerRows.length; i++ ){
        managerArgs.push( managerRows[i].id );
    }
    var managers = global.dbSelect(global.DB_TABLE_USERS, '`id`, `first`, `last`', [{val:managerArgs}]).rows;
    ...
    //console.log(data);
    res.render('projects/table', data);
